...
from projects.rock_paper_scissors.rock_paper_scissors import RockPaperScissorsGame, Player, GameOptions
...
@pytest.mark.parametrize('user_choice, computer_choice, expected_winner', [
...
def test_determine_winner(user_choice: str, computer_choice: str, expected_winner: Player):
...
def test_get_computer_choice():
    ...
    assert game.get_computer_choice() in tuple(
        option.value for option in GameOptions)
