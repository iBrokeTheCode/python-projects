...
from projects.unit_converter.unit_converter import parse_arguments, convert_units, main


def test_parse_arguments():
    sys.argv = ['unit_converter.py', '--category', 'length',
                '--from', 'meters', '--to', 'centimeters', '--value', '1']
...
@pytest.mark.parametrize('category, from_unit, to_unit, value, expected', [
...
@pytest.mark.parametrize('category, from_unit, to_unit, value', [
...
def test_main_invalid_unit(capsys):
    sys.argv = ['unit_converter.py', '--category', 'length',
                '--from', 'meter', '--to', 'centimeters', '--value', '1']
    main()
    captured = capsys.readouterr()
